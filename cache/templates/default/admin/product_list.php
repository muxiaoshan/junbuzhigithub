...
<? if($one['type']=='ticket') { ?>
<a href="?mod=coupon&code=vlist&search=pid:<?=$one['id']?>&ssrc=product_name&sstr=<?=$one['flag']?>">[ 查看团购券 ]</a><? } elseif($one['type']=='stuff') { ?><a href="?mod=delivery&code=vlist&alsend=no&search=pid:<?=$one['id']?>&ssrc=product_name&sstr=<?=$one['flag']?>">[ 发货管理 ]</a><? } elseif($one['type']=='prize') { ?><a href="?mod=prize&code=mgr&pid=<?=$one['id']?>">[ 抽奖管理 ]</a>
<? } ?>
<? } ?>
</td> <td>
