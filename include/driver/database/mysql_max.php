...
class mysql_maxDatabaseDriver
{
	...
	public function is_sql_safe($sql)
	{
		static $status = null;
	...
	private function _do_query_safe($sql)
	{
		...
		if(is_array($_CONFIG['dfunction']))
		{
			...
			{
				if(strpos($clean, $fun.'(') !== false) return array('r' => 'dfunction', 'm' => $fun);
			}
		}
		if(is_array($_CONFIG['daction']))
		{
			...
			{
				if(strpos($clean, $action) !== false) return array('r' => 'daction', 'm' => $action);
			}
		}
	...
	private function sql_hack_found($data)
	{
